...
The tree is a Cargo workspace (`crates/core`, `crates/docscheck`, `crates/gamchi`, `crates/adapter-grok`). `make test`
...
and fail-closes mid-thread model change. TASK-027 live model/effort proof is
`cargo test -p samchi-adapter-grok --test live_model -- --ignored`. TASK-007's live
...
Focused lane: `cargo test -p samchi-core`, `cargo test -p docscheck`, `cargo test -p gamchi`, or `cargo test -p samchi-adapter-grok` for one
package. CLI worker and MCP stdio tests in `gamchi` drive the fake
ACP agent via `GAMCHI_ACP_PROGRAM`. There is no aggregate integration or e2e target
...
`cargo test -p gamchi --test live_followup -- --ignored`.
The second turn uses the same ACP session id.
TASK-013 live respond is
`cargo test -p gamchi --test live_respond -- --ignored`.
...
`userAgent=gamchi/app-server-v1`, `capabilities.ccas` is `-32602`,
...
agent via `GAMCHI_ACP_PROGRAM`). Omitted socket fields stay
