...
| --- | --- | --- | --- |
| JSON-RPC MCP client | yes | yes (`TASK010_LIVE.txt`) | `gamchi mcp --home <tmp>` |
...
| Codex | yes (`codex`) | yes (`TASK010_CODEX.txt`, ledger terminal) | `codex exec -c mcp_servers.gamchi.*`; no `~/.codex/config.toml` edit |
