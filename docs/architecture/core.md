...
TASK-024 reads home `config.yaml` (`default_model` / `default_effort`) then
built-in `grok-4.6` / `high`, puts `-m` and `--reasoning-effort` on `agent`,
and fails the turn on child start or ACP failure. `grok models` does not
gate spawn.
